	...
	*/
	public function fetchStoresData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if($value['tipe']==1){$tipe = 'Outlet';}elseif($value['tipe']==2){$tipe = 'Office';}elseif($value['tipe']==3){$tipe = 'Logistik';}else{$tipe ='Tidak Diketahui';}
    ...
    */
	public function update($id)
	{
		...
		if($id) {
			...
			$this->form_validation->set_rules('edit_tipe', 'Tipe', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
	        		...
	        		'tipe' => $this->input->post('edit_tipe')	
	        	);
